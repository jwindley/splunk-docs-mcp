...
- [ ] Update `setup.py`:
  ...
  - Default mode: display numbered menu of sources with page counts + sizes; accept comma-separated selection or `'all'`
  - `--all` flag: skip menu; print total size warning + confirmation prompt
  - Download selected per-source DBs to `.tmp` files; merge via `merge_dbs()`; atomic rename to `data/splunk_docs.db`
- Note: `manifest.json` is already generated by `splunk-merge --export-sources`; schema is `{generated_at, total_pages, sources: [{source_id, display_name, version, pages, chunks, file_name, size_bytes}]}`
